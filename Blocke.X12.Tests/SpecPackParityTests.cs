...
public sealed class SpecPackParityTests
{
    [Fact]
    ...
    public void Unpack_parses_in_predicate_values_correctly()
    {
        ...
        var seg = Assert.IsType<SegmentNodeSpec>(Assert.Single(spec.Root));
        var must = Assert.IsType<InSetPredicate>(Assert.Single(seg.Rules).Must);
        Assert.Equal(new HashSet<string>(["BT", "ST", "VN", "SN"]), must.Values);
    }
    ...
    [Fact]
    public void Rule_severity_parse_is_case_insensitive()
    {
        var spec = SpecPack.Unpack([
            "X|850|005010",
            "SEG|BEG|M|1",
